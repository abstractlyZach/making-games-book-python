...
class BoardView(object):
    ...
    def __init__(self, display_surface, box_size, gap_size, x_margin, y_margin,
                 background_color):
    ...
    def _draw_icon(self, x, y):
        ...
        """
        left, top = self.left_top_box_coords(x, y)
    ...
    def _draw_donut(self, color, left_x, top_y):
        ...
        quarter = math.floor(self._box_size / 4)
        half = math.floor(self._box_size / 2)
        pygame.draw.circle(self._display_surface, color,
                           (left_x + half, top_y + half),
                           half - 5)
